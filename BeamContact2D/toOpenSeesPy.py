    ...
    for line in infile:
        ...
        if info[1] == 'forceBeamColumn' or info[1] == 'dispBeamColumn':
            ...
            if info[1] == 'dispBeamColumn':
                outfile.write(
                    'ops.beamIntegration(\'Legendre\',%s,%s,%s)\n' % (
                    eleTag, secTag, Np))
            if info[1] == 'forceBeamColumn':
                outfile.write(
                    'ops.beamIntegration(\'Lobatto\',%s,%s,%s)\n' % (
                    eleTag, secTag, Np))
        ...
        for i in range(2, N):
            ...
            if info[0] == 'recorder':
                ...
                if info[i - 1] in ['material', 'section',
                                   'fiber'] and isfloat(info[i]):
...
if __name__ == '__main__':
    outfile = open('excavation.py', 'w+')
    toOpenSeesPy('excavation.tcl', outfile)
